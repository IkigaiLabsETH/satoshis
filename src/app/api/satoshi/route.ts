// ---
// Satoshi API Route (Modular Service Architecture)
//
// This route implements the Satoshi multi-modal persona API endpoint.
//
// All major logic is now delegated to dedicated service modules in ./services:
//   - Market data integration: services/satoshiMarketData.ts
//   - Persona routing/normalization: services/satoshiPersona.ts
//   - Prompt construction and context trimming: services/satoshiPromptBuilder.ts
//   - Error and timeout formatting: services/satoshiUtils.ts
//
// This modular structure improves maintainability, testability, and clarity.
// ---
...
// Add imports for new service modules
import { normalizePersonaMode } from './services/satoshiPersona';
import { buildPromptContext } from './services/satoshiPromptBuilder';
import { formatLLMTimeoutResponse, formatAPIErrorResponse } from './services/satoshiUtils';
...
interface _InsiderSentiment {
  data?: Array<{ mspr?: number; month?: string; year?: string }>;
}

interface _CompanyEarnings {
  epsActual?: number;
  epsEstimate?: number;
  date?: string;
...
interface _IPOCalendar {
  ipoCalendar?: Array<{ name?: string; symbol?: string; date?: string }>;
}

interface _CompanyNews {
  headline: string;
  datetime: string;
}

interface _AnalystRecommendation {
  buy?: number;
  hold?: number;
  sell?: number;
  strongBuy?: number;
...
interface FetchRelevantMarketDataResult {
  marketData: string;
  webSearch: string;
  xSentiment: string;
...
// --- Helper: Fetch all relevant market data and context in parallel ---
async function fetchRelevantMarketData(input: string, sources: string[], symbol: string): Promise<FetchRelevantMarketDataResult> {
  let marketData = '';
  let webSearch = '';
  let xSentiment = '';
  let satoshiMarket = '';
  ...
  if (shiftedResults[0] && shiftedResults[0].status === 'fulfilled' && typeof shiftedResults[0].value === 'string') {
    marketData = shiftedResults[0].value;
    used.push('Coingecko');
  } else if (sources.includes('coingecko')) {
    warning += '⚠️ CoinGecko price fetch failed or timed out. ';
  }
  if (shiftedResults[1] && shiftedResults[1].status === 'fulfilled' && typeof shiftedResults[1].value === 'string') {
    webSearch = shiftedResults[1].value;
    used.push('Web Search');
  } else if (sources.includes('web')) {
    warning += '⚠️ Web search failed or timed out. ';
  }
  if (shiftedResults[2] && shiftedResults[2].status === 'fulfilled' && typeof shiftedResults[2].value === 'string') {
    xSentiment = shiftedResults[2].value;
  } else if (sources.includes('web')) {
    warning += '⚠️ X sentiment fetch failed or timed out. ';
  }
  if (shiftedResults[3] && shiftedResults[3].status === 'fulfilled' && shiftedResults[3].value && typeof shiftedResults[3].value === 'object' && 'c' in shiftedResults[3].value) {
  ...
  if (shiftedResults[4] && shiftedResults[4].status === 'fulfilled' && shiftedResults[4].value && typeof shiftedResults[4].value === 'object' && 'data' in shiftedResults[4].value) {
  ...
  if (shiftedResults[5] && shiftedResults[5].status === 'fulfilled' && Array.isArray(shiftedResults[5].value)) {
  ...
  if (shiftedResults[6] && shiftedResults[6].status === 'fulfilled' && shiftedResults[6].value && typeof shiftedResults[6].value === 'object' && 'ipoCalendar' in shiftedResults[6].value) {
  ...
  if (shiftedResults[7] && shiftedResults[7].status === 'fulfilled' && Array.isArray(shiftedResults[7].value)) {
  ...
  if (shiftedResults[8] && shiftedResults[8].status === 'fulfilled' && Array.isArray(shiftedResults[8].value)) {
  ...
  if (shiftedResults[9] && shiftedResults[9].status === 'fulfilled' && shiftedResults[9].value && typeof shiftedResults[9].value === 'object' && 'targetHighPrice' in shiftedResults[9].value) {
  ...
  if (shiftedResults[10] && shiftedResults[10].status === 'fulfilled' && typeof shiftedResults[10].value === 'string') {
...
// --- Special-case handler: Portfolio Simulation ---
async function handlePortfolioSimulation(input: string, persona: string, used: string[]) {
  ...
  return NextResponse.json({ persona, prompt: '', processed: answer, dataSourceUsed: used });
}

// --- Special-case handler: Earnings Comparison ---
async function handleEarningsComparison(input: string, persona: string, used: string[]) {
  const JAN1_PRICES = {
    BTC: 45000,
    NVDA: 480
  };
...
// --- Special-case handler: Analyst Compare ---
async function handleAnalystCompare(input: string, persona: string, prompt: string, llmMaxTokens: number) {
  ...
  return NextResponse.json({ persona, prompt: prompt, processed: fallbackProcessedString, dataSourceUsed: ['Finnhub', 'CoinGecko'] });
}
